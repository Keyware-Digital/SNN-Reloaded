...
init_weapons()
{
	...
	add_zombie_weapon( "mosin_rifle", 							&"ZOMBIE_WEAPON_MOSIN_200", 				200,	"vox_crappy", 0); 
...
treasure_chest_ChooseRandomWeapon( player )
{
	...
	for( i = 0; i < keys.size; i++ )
	{
		...
		if(keys[i] == "zombie_bowie_flourish")
		{
			if(player hasperk("specialty_altmelee") || player.has_altmelee)
			{
				continue;
			}
		}
	...
	else if( primaryWeapons.size == 1 ) 
	{
		...
		if( isdefined( current_weapon ) )
		{
			if( !( weapon_string == "fraggrenade" || weapon_string == "stielhandgranate" || weapon_string == "molotov" ) )
			{
				...
				if(!(current_weapon == "zombie_colt" || current_weapon == "colt" || current_weapon == "nambu" || current_weapon == "sw_357" || current_weapon == "tokarev"))
				self TakeWeapon( current_weapon ); 
			}
		} 
...
weapon_give( weapon )
{
	...
	if( IsDefined( primaryWeapons ) && !isDefined( current_weapon ) )
	{
		for( i = 0; i < primaryWeapons.size; i++ )
		{
			if( primaryWeapons[i] == "zombie_colt" || primaryWeapons[i] == "nambu" || primaryWeapons[i] == "tokarev" || primaryWeapons[i] == "walther" )
			{
				continue; 
			}
